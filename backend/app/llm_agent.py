...
class LLMAgent:
    ...
    async def suggest_commands(
        ...
        is_llm_mode = "/llm" in user_message.lower() or error_context is None
        
        if is_llm_mode:
            system = (
                "You are a command translator for a Gantt chart planner. "
                "The user wants to modify their plan. "
                "Translate their intent into executable fast commands.\n\n"
                "Available fast commands (use EXACTLY these formats):\n"
                "- 'добавь задачу {name}' — create task\n"
                "- 'добавь задачу {name} на {N} дней' — create with duration\n"
                "- 'добавь задачу {name} с исполнителем {person}' — create with assignee\n"
                "- 'добавь задачу {name} на {N} дней с исполнителем {person}' — create with both\n"
                "- '{name} связана с {dep}' — link tasks\n"
                "- '{name} сдвинь на {N}' — shift task by N days\n"
                "- '{name} сдвинь на {N} назад' — shift backward\n"
                "- '{name} перенеси на {YYYY-MM-DD}' — move to absolute date\n"
                "- '{name} назначь {person}' — assign person\n"
                "- '{name} удали' — delete task\n"
                "- '{name} скопируй' — duplicate task\n\n"
                "CRITICAL RULES:\n"
                "- ALWAYS use lowercase for command verbs: 'добавь', 'сдвинь', 'назначь', 'удали', 'скопируй'\n"
                "- NEVER use infinitive forms like 'добавить', 'сдвинуть', 'назначить'\n"
                "- Task name goes BEFORE the verb for shift/copy/delete/assign: 'Full-stack сдвинь на 3'\n"
                "- If user mentions a person/assignee, use 'с исполнителем {person}'\n"
                "- If user mentions duration (days), use 'на {N} дней'\n"
                "- Output ONLY valid JSON\n"
                "- Format: {\"suggestions\": [{\"label\": \"Button text\", \"command\": \"actual command\"}], \"note\": \"Brief explanation in Russian\"}\n"
                "- Commands must use exact fast command syntax\n"
                "- Include ALL info from user message\n"
                "- Multiple commands = multiple suggestions\n"
            ...
            system = (
                ...
                "- '{name} скопируй' — duplicate task\n\n"
                "CRITICAL RULES:\n"
                "- You are a translator, NOT an executor. Do NOT claim tasks are created.\n"
                "- Output ONLY valid JSON\n"
                "- Format: {\"suggestions\": [{\"label\": \"Button text\", \"command\": \"actual command\"}], \"note\": \"Brief explanation in Russian\"}\n"
                "- Commands must use exact fast command syntax\n"
                "- Include ALL info from user message (assignee, dates, etc.)\n"
                "- If user mentions dates, compute the actual YYYY-MM-DD\n"
                "- Multiple commands = multiple suggestions\n"
                "- Respond note in Russian\n\n"
                f"Current plan:\n{plan_context}\n\n"
